 ...
 *
 * 실행: cd app && node tools/build-guardian-now.mjs → app/public/guardian-holo.html
 * ───────────────────────────────────────────────────────────────────── */
...
/* ── 홀로그램 실험 ──────────────────────────────────────────────────
   가설: **입자를 버리지 않아도 그 룩이 나온다.** 점을 키우고 알파를 낮추면
   낱알이 겹쳐 연속 그라데이션이 된다 — 셰이더 한 줄도 안 고치고 상수 셋만 바꾼다.
   ps=점 크기 배수 · al=알파 배수 · sm=번짐 패스 가중치 */
const COLS = [
  { key: "now",  title: "현행", sub: "낱알이 보인다",        orb: 1, gy: 0, ps: 1,    al: 1,   sm: 1 },
  { key: "mid",  title: "중간", sub: "낱알 + 번짐",          orb: 1, gy: 0, ps: 3.2,  al: 0.42, sm: 2.2 },
  { key: "holo", title: "홀로그램", sub: "낱알이 안 보인다", orb: 1, gy: 0, ps: 7.0,  al: 0.18, sm: 3.0 },
];
...
function init(){
  ...
  document.querySelectorAll("canvas").forEach(cv=>{
    ...
    const n=5000, r0=new Float32Array(n*4), r1=new Float32Array(n*4), rnd=srnd(20260826);
    ...
    f1("u_form",el.form); f1("u_ps",1.8*dpr*(F_PS[el.key]||1)*(col.ps||1)); f1("u_orb",col.orb);
    ...
    let stop=false; setTimeout(()=>{stop=true;window.__frozen=true;},3000);
    (function loop(){
      ...
      const P=col.ps||1, AL=col.al||1, SM=col.sm||1;
      f1("u_psMul",3.6*SM); f1("u_alpha",0.05*A*AL); gl.drawArrays(gl.POINTS,0,n);
      f1("u_psMul",1.8*SM); f1("u_alpha",0.22*A*AL); gl.drawArrays(gl.POINTS,0,n);
      f1("u_psMul",1);      f1("u_alpha",0.72*A*AL); gl.drawArrays(gl.POINTS,0,n);
    })();
  });
}
...
`;
const OUT=resolve(APPDIR,"public/guardian-holo.html");
