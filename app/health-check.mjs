...
/* ── 검사 5-r. 템플릿 문자열 안에 백틱이 있는가 (2026-08-28) ───────────────
   CSS·셰이더는 **백틱 템플릿 리터럴**에 들어 있다. 그 안 주석에서 코드를 백틱으로 인용하면
   **문자열이 거기서 끊기고** 뒤가 JS 로 해석된다. 빌드가 잡긴 하는데 메시지가
   「Expected ";" but found …」라 원인이 안 보인다.
   ⚠ **검사는 이미 있었는데(`e2e/css-backtick-check.mjs`) 검진이 안 불렀다.**
     그래서 한 세션에서만 세 번 같은 실수를 했다(셰이더 1 · CSS 2). 배포 직전 관문에 넣는다. */
{
  const bad = [];
  if (existsSync("e2e/css-backtick-check.mjs")) {
    try { execFileSync("node", ["e2e/css-backtick-check.mjs"], { stdio: "pipe", timeout: 30000 }); }
    catch (_) { bad.push("템플릿 안에 백틱이 있음 — node e2e/css-backtick-check.mjs"); }
  } else bad.push("백틱 검사 파일이 없음");
  add(bad.length ? "심각" : "정상",
    bad.length ? "CSS·셰이더 문자열이 중간에 끊길 수 있음" : "템플릿 문자열 — 안에 백틱 없음",
    bad.length ? bad.join(" · ") : "CSS·셰이더 템플릿 깨끗",
...
/* ── 검사 5-p. 조사가 값에 따라 갈리는가 (2026-08-26) ──────────────────────
   창업자 제보("오타") — 각인 문서에 **"돈 쥐기이 얇아"** 가 떠 있었다.
   `{r.core.block.t}이 얇아` 로 조사를 박아 뒀는데, 그 자리 이름 다섯 중 **둘이 받침이 없다**
   (돈 쥐기 · 도움받기). 같은 파일에 `josa()` 헬퍼가 이미 있는데 그 자리만 안 썼다.

   훑어보니 **같은 모양이 명식 리포트에 다섯 군데 더** 있었다 —
   "불가 3개로 몰려 있어" · "몰아치는 압박가 오는 열 해야" · "물를 치는 기운이라".
   전부 무료 화면이고, 유저가 제일 먼저 보는 문서다.

   ⚠ 이건 **한 번 고치고 끝나는 종류가 아니다.** 새 문장을 쓸 때마다 다시 생긴다.
     그래서 규칙으로 세운다: **보간값 뒤에 조사를 그대로 붙이지 마라.**
   ⚠ 예외 둘은 값이 실제로 안 갈려서 통과시킨다(아래 SAFE). 예외를 늘리려면
     그 값 집합이 **전부 받침이 있거나 전부 없는지** 먼저 확인할 것. */
{
  const app = readFileSync(APP, "utf8");
  /* 값이 안 갈리는 자리 — cost 는 전부 「것」·「값」(받침 O), cat 은 A|B|C 라틴이고 프롬프트 안이다 */
  const SAFE = [/\{cost\}을/, /\{_prevRec\.cat \|\| "A"\}를/];
  const bad = [];
  for (const m of app.matchAll(/\{([^{}]{1,60})\}(이|가|은|는|을|를|와|과)(?=[ .,·\n<*])/g)) {
    const expr = m[1].trim();
    if (/josa|jong|IYA|\?/.test(expr)) continue;          // 이미 갈라 쓰고 있으면 통과
    if (SAFE.some((re) => re.test(m[0]))) continue;
    bad.push(m[0].slice(0, 40));
  }
  add(bad.length ? "심각" : "정상",
    bad.length ? "값 뒤에 조사가 박혀 있음 — 받침에 따라 문장이 깨진다" : "조사 — 보간값 뒤에 고정 조사 없음",
    bad.length ? bad.join(" · ") : `예외 ${SAFE.length}건(값이 안 갈리는 자리)만 허용`,
    "`{값}이` 처럼 조사를 박으면 **받침 없는 값에서 「돈 쥐기이 얇아」가 됩니다.** 같은 파일의 `josa(값, \"이\", \"가\")` 를 쓰세요. 값 집합이 전부 받침이 있거나 전부 없다면 위 SAFE 에 근거와 함께 등재하세요.");
}
