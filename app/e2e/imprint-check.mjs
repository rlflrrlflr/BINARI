...
/* ⚠ **각주는 「용어 병기」 규칙의 예외다 — 창업자 결정 2026-08-29.**
   문장 감사가 「각주 68개에 무병기 용어가 있다」(「일간」 11회 등)를 지적했는데,
   이 검사는 정반대로 **각주에 기법 이름을 적으라고 요구한다.** 결함이 아니라 두 규칙이 부딪히는 자리였고,
   창업자가 *"우선 각주는 예외로 하자"* 로 정했다.
   → **각주에서 용어를 걷어내지 마라.** 각주는 유저를 위한 설명이 아니라 **검산을 위한 출처 표시**다 —
     기법 이름이 빠지면 그 값이 어디서 나왔는지 아무도 못 되짚는다(본문은 병기 규칙 그대로 유지). */
ck("각주에는 기법 이름을 적는다(검증용이므로 · 병기 규칙의 예외)", a.notes.join(" ").includes("일간"), "");
