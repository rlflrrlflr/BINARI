...
// ── v113 각인 — 판결 흐름 밖의 두 번째 상품. 로비에서 열린다(결제 전이라 지금은 무료 발행)
{
  ...
  /* ── v125 궁합 — 각인의 애드온. 생년월일 + (v136부터) 부를 이름을 받고 연락처는 안 받는다 ── */
  {
    ...
    if (await mb.count()) {
      ...
      /* ⚠ 2026-08-31 — 탐침을 바꿨다. 예전 문장은 문서 **맨 끝에서** 「이 숫자를 먼저 보지 마」라고
         읽는 순서를 지시했는데, 여기까지 내려온 사람에게는 이미 늦은 말이고 **우리 편집 사정**이었다.
         검사가 재는 것은 그대로다 — 총점이 맨 뒤에 있고, 그 옆에 숫자를 떠받들지 말라는 경고가 붙는가.
         한 줄을 더한 이유: 경고가 **갈 곳을 주는지**까지 물어야 선언으로 되돌아가지 않는다. */
      ck("궁합 — 총점을 맨 뒤에 두고 숫자를 떠받들지 말라고 한다", /이 숫자로 사람을 고르진 마/.test(mt));
      ck("궁합 — 그 경고가 갈 곳을 준다(선언으로 끝내지 않는다)", /「조심할 것」이 네 경우의 답/.test(mt));
      ck("궁합 — 헤어지라고 안 한다", !/(헤어져|정리해|만나지 마|그만 만나)/.test(mt));
      /* ⚠ **2026-08-30 뜻을 뒤집었다.** 이 줄은 궁합이 유료·무게이트였던 v130 의 「재구매 논리」
         검사다. 창업자 게이트(첫 곁만 직접 입력, 그 다음은 초대)가 서면서 **이 버튼의 존재 자체가
         게이트 위반**이 됐다 — 실측으로 초대 0건에 다섯 명까지 직접 입력됐다.
         이 시점의 곁은 방금 든 한 명이므로 문은 닫혀 있어야 한다. */
      ck("궁합 — 첫 곁이 선 뒤에는 직접 입력 문이 닫힌다", !/다른 사람과도 봐볼게/.test(mt));
      ck("궁합 — 닫힌 이유를 그 자리에서 말한다", /다음 사람은 그 사람이 직접 넣어야 서/.test(mt));
