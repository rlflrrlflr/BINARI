     ...
     ② **총점이 여기 안 들어간다**(평균을 내면 이 절이 부정하는 바로 그것이 된다) */
{
  ...
  for (let i = 0; i < 3000; i++) {
    ...
    /* ⚠ 탐침 문자열은 2026-08-31 에 바뀌었다 — 머리글의 주어를 문명 이름에서
       **둘 사이의 영역**으로 옮기면서 세 갈래가 전부 새로 쓰였다(창업자 지적: 문명이 주어면
       그건 우리 사정이다). 검사가 재는 것은 그대로 **세 갈래가 다 도달 가능한가**다. */
    if (/어긋나는 데가 한 군데도/.test(h)) kinds.한목소리++;
    else if (/맞는 데가 한 군데도/.test(h)) kinds.다어렵다++;
    else if (/안에서도 답이 둘/.test(h)) kinds.머리글이내부갈림++;
    else kinds.갈림++;
    if (x.chorus.inner) kinds.덧줄++;
    /* ⚠ **탐침이 문장 모양을 따라가야 한다 (2026-08-31).** 머리글을 새로 쓰면서 옛 정규식이
       아무것도 안 물게 됐다 — 그대로 뒀으면 이 검사는 **늘 통과하는 장식**이 됐을 것이다.
       두 가지를 문다: ①양쪽이 같은 영역인 문장 ②양쪽이 같은 문명인 문장. */
    const m = h.match(/^(.+?)(?:은|는) 맞는데 (.+?)(?:은|는) 갈려 — 재는 자리가 달라서야\((.+?) ↔ (.+?)\)/);
    if (m && (m[1] === m[2] || m[3] === m[4])) { kinds.버그 = (kinds.버그 || 0) + 1; }
    /* ⚠ **덧줄이 머리글의 영역을 되풀이하면 두 번째 줄은 정보가 0이다** — 실측 80.3% 였다.
       겹칠 바에는 덧줄을 안 붙이는 게 맞고, 엔진이 그렇게 고쳐졌다. 여기서 되돌아오는지 본다. */
    if (m && x.chorus.inner) {
      const mi = st(x.chorus.inner).match(/— (.+?)(?:은|는) 맞고 (.+?)(?:은|는) 갈려/);
      if (mi && [mi[1], mi[2]].some((d) => d === m[1] || d === m[2])) kinds.겹침 = (kinds.겹침 || 0) + 1;
    }
  }
  ...
  ck("⑩ 머리글 양쪽이 같은 영역·같은 문명이 되지 않는다", !kinds.버그, `${kinds.버그 || 0}건`);
  ck("⑩ 덧줄이 머리글의 영역을 되풀이하지 않는다", !kinds.겹침, `${kinds.겹침 || 0}건`);
}
