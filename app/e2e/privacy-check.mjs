        ...
        나란히 놓고 값을 정할 수 없는 두 숫자였다. 분모(노출 이벤트)도 한쪽에만 있었다. */
{
  ...
  /* v128.1: 값의 쓰임이 '수호신 질감 코드' 하나뿐이라 중립 이름(tex)으로 이관했다.
     보는 것은 이름이 아니라 **옛 유저 얼굴이 보존되는가** — 저장된 값을 읽는 폴백과,
     그 값이 없을 때만 파생하는 분기가 둘 다 살아 있어야 한다. */
  ck("D-1 수호신 얼굴 시드로는 계속 쓴다(옛 유저 얼굴 보존)",
     /tex \|\| texture\(/.test(src) && /mem\?\.tex \|\| mem\?\.mbti/.test(src));
