      ...
      파일 전체를 보면 그걸 잡아 헛울음이 난다. **나가는 것과 남는 것은 다르다.** */
{
  ...
  /* ⚠ 2026-08-28: 체크박스 동의 → **행위 동의**로 바뀌었다(창업자 "선택지 없애").
     검사도 같이 옮긴다 — 지우지 않고 **무엇으로 동의를 받는지**를 묻는다. */
  ck("§5-2 가 무엇으로 동의를 받는지 밝힌다(고지 + 버튼)",
     /무엇이 전달되는지 화면에서 먼저 알리며, 그 버튼을 누르는 것이 동의입니다/.test(pv2));
  ck("§5-2 가 전달을 원치 않을 때의 길을 밝힌다",
     /제출하지 않으면 됩니다/.test(pv2) && /보낸 이에게는 아무것도 전달되지 않습니다/.test(pv2));
  /* ⚠ **이 줄의 약속이 2026-08-28 에 바뀌었다.** 예전엔 "받은 이의 명식은 **어느 경우에도**
     보낸 이에게 안 간다"였는데, 그 선을 지키면 보낸 이가 초대를 하고도 그 사람과의 사이를
     영영 못 본다(창업자 결정으로 뒤집힘). 그래서 **새 경계를 여기서 다시 못 박는다** —
     느슨해진 게 아니라 **자리가 옮겨진 것**이고, 옮겨진 자리를 검사가 안 물면 다음 판에 더 샌다:
       ① B의 값은 **동의했을 때만** 간다  ② 생년월일 원값은 **어느 방향으로도** 안 간다
       ③ 동의를 안 하면 **답했다는 사실조차** 안 간다(예전의 「한 비트」도 이제 안 간다)
       ④ 되짚을 수 있는 사람은 **상대 한 사람뿐**이다(제3자 아님) */
  ck("§5-2 — B의 값이 언제 가는지 밝힌다",
     /받은 이가 생년월일을 제출하면[\s\S]{0,200}보낸 이도 두 사람 사이를 볼 수 있게/.test(pv2));
  ck("§5-2 — 이름도 간다는 걸 숨기지 않는다", /본인이 적은 표시용 이름<\/strong>/.test(pv2));
  ck("§5-2 — 생년월일 원값은 어느 방향으로도 안 간다",
     /생년월일·태어난 시각의 원래 값은 어느 방향으로도 가지 않습니다/.test(pv2));

  ck("§5-2 — 되짚을 수 있는 건 상대 한 사람뿐임을 밝힌다",
     /되짚기가 가능한 사람은 <strong>상대방 한 사람뿐<\/strong>/.test(pv2));
  /* ⚠ **A(보낸 이)의 이름도 서버에 간다 (2026-08-29 신설).**
     `inviteNew()` 가 `{ axes, name: birth.name }` 를 올리고 서버가 30일 보관한 뒤 B 에게 돌려준다.
     그런데 §5-2 「저장하는 것」 칸은 **B 의 이름만** 적고 있었다 — A 것은 빠져 있었다.
     기존 검사 셋(파생값 목록 대조)은 **축 이름만** 물어서 이 구멍을 못 봤다.
     그래서 여기서는 축이 아니라 **「무엇이 저장되는가」의 목록 자체**를 문다. */
  ck("§5-2 — 보낸 이의 이름이 저장된다는 것도 밝힌다",
     /초대를 보낸 이용자의[\s\S]{0,400}표시용 이름/.test(pv2));
  ck("§5-2 — 그 이름이 어디에 쓰이는지까지 밝힌다(무엇이 나가는지 보이게)",
     /받은 사람 화면에[\s\S]{0,80}뜨는 이름/.test(pv2));
  /* 화면이 그 약속대로 말하는가 — **문서만 고치고 화면을 두면 유저가 보는 문장이 거짓이 된다.**
     v127.7 전례가 정확히 그것이고, 이번엔 방향이 반대(코드가 먼저 가고 약속이 낡는)일 뿐이다. */
  {
    const inv = (src.match(/function InviteLanding[\s\S]*?\n\}/) || [""])[0];
    /* ⚠ **동의 체크박스가 2026-08-28 에 사라졌다**(창업자: "선택지 없애"). 체크가 없어졌다고
       동의까지 없어지면 안 된다 — 받은 사람은 이 앱을 쓴 적 없는 제3자다. 그래서 **행위 동의**로
       옮겼고(고지 + 버튼), 검사도 그 자리로 옮긴다. 옛 검사를 지우기만 하면 다음 판에 고지까지 사라진다. */
    ck("§5-2 ↔ B 화면 — 무엇이 가는지 버튼 위에 적혀 있다",
       /누르면 <b>\{who\}도 둘 사이를 보게 돼/.test(inv));
    ck("§5-2 ↔ B 화면 — 무엇이 가고 무엇이 안 가는지 둘 다 적는다",
       /네 생일은 이 기기에만 남아/.test(inv) && /네가 적은 이름과 둘 사이를 계산하는 값/.test(inv));
    ck("§5-2 ↔ B 화면 — 고를 수 있는 척하지 않는다(체크박스가 없다)",
       !/invchk/.test(inv) && !/type="checkbox"/.test(inv));
    ck("§5-2 ↔ B 화면 — 되짚기 가능성을 화면이 숨기지 않는다",
       /되짚을 수는 있어/.test(inv));
    /* 방침이 「제출하지 않으면 아무것도 안 간다」고 말하므로, **제출 전에는 값이 안 나가야** 한다.
       엿보기(GET)만 나가고 좌표는 제출 시점에만 실린다 — 그건 invite-landing-check ① 이 실측한다. */
    ck("§5-2 ↔ B 화면 — 제출 전에는 좌표를 안 만든다",
       /bAxes: matchAxes/.test(inv) && inv.indexOf("bAxes") > inv.indexOf("const submit"));
  }
