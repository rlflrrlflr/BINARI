      ...
      파일 전체를 보면 그걸 잡아 헛울음이 난다. **나가는 것과 남는 것은 다르다.** */
{
  ...
  ck("§5-2 — B의 값은 동의했을 때만 간다",
     /받은 이가 동의하면[\s\S]{0,120}보낸 이도 두 사람 사이를 볼 수 있게/.test(pv2));
  ck("§5-2 — 생년월일 원값은 어느 방향으로도 안 간다",
     /생년월일·태어난 시각의 원래 값은 어느 방향으로도 가지 않습니다/.test(pv2));
  ck("§5-2 — 동의 안 하면 답했다는 사실조차 안 간다",
     /답했다는 사실조차 가지 않습니다/.test(pv2));
     ...
     v127.7 전례가 정확히 그것이고, 이번엔 방향이 반대(코드가 먼저 가고 약속이 낡는)일 뿐이다. */
  {
    ...
    ck("§5-2 ↔ B 화면 — 체크 문구가 「둘 사이를 함께 본다」로 말한다",
       /둘 사이를 보게 할까/.test(inv) && !/답했다는 걸 \{who\}에게 알려도 될까/.test(inv));
    ck("§5-2 ↔ B 화면 — 되짚기 가능성을 화면이 숨기지 않는다",
       /되짚을 수는 있어/.test(inv));
    ck("§5-2 ↔ B 화면 — 끄면 아무것도 안 나간다고 말하고, 실제로 안 싣는다",
       /지금은 아무것도 안 나가/.test(inv) && /notify\s*\n?\s*\?\s*\{ id, notify, bAxes/.test(inv.replace(/\s+/g, " ").replace(/notify \? \{ id, notify, bAxes/, "notify ? { id, notify, bAxes")));
  }
