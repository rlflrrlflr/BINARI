      ...
      파일 전체를 보면 그걸 잡아 헛울음이 난다. **나가는 것과 남는 것은 다르다.** */
{
  ...
  /* 같은 이유로 여기도 옮겼다 — 「한 사람뿐」이 「응답한 사람들」이 됐다 */
  ck("§5-2 — 되짚을 수 있는 건 응답한 사람들임을 밝힌다",
     /되짚기가 가능한 사람은 <strong>그 링크를 받아 응답한 사람들<\/strong>/.test(pv2)
     && /그 사람들 모두가<\/strong>/.test(pv2));
  ...
  /* ⚠ **2026-08-30 에 뒤집혔다.** 「한 번만 응답」이 창업자 결정으로 「여럿이 답한다」가 됐다
     (*"단톡방에 가면 여러 사람이 참여할 텐데 지금은 한 명만 가능한 것 같다"*).
     그 문장이 지키던 건 **A의 좌표를 받는 사람 수**였으므로(좌표는 되짚을 수 있다),
     검사를 지우지 않고 **새 경계로 옮긴다** — 지우기만 하면 다음 판에
     「누가 얼마나 받는지」를 아무 데서도 안 말하게 된다. */
  ck("§5-2 가 여럿이 답할 수 있음과 상한을 밝힌다",
     /받은 사람 누구나 응답할 수 있으며, 한 링크에 최대 24명/.test(pv2));
  ck("§5-2 가 단체 대화방으로 퍼질 때를 밝힌다",
     /그 링크를 받은 사람 모두가 보낸 이의 파생값을 받게 됩니다/.test(pv2));
  ck("§5-2 가 끊는 방법을 밝힌다", /취소하면 즉시 삭제되어 그 뒤로는 아무도 응답할 수 없습니다/.test(pv2));
  ck("§5-2 가 만료를 밝힌다(30일)", /30일 뒤 만료<\/strong>됩니다/.test(pv2));
  ck("(옛 약속) 「한 번만 응답」이 문서에 안 남아 있다", !/한 번만 응답되고 만료/.test(pv2));
  /* ⚠ **처리방침은 열어 보는 사람만 읽는다.** 단톡방에 뿌리기 직전 화면에 안 적혀 있으면
     그 고지는 아무에게도 안 닿는다 — 그래서 화면도 같이 문다. */
  ck("§5-2 ↔ 곁 화면 — 뿌리기 전에 링크의 성질을 말한다",
     /받은 사람 누구나 답할 수 있어/.test(src) && /되짚을 수도 있어/.test(src));
