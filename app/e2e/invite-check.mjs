...
async function call(method, { seg = [], query = {}, body = null, origin = ORIGIN } = {}) {
  const req = { method, headers: { origin }, query: { ...query, seg }, body };
...
{
  _resetMem();
  const c = await call("POST", { body: { axes: AXES, name: "민수" } });
  ...
  const after = await call("GET", { query: { ids: id } });
...
/* ── ③ 응답 1회 — 재공유 방어 (지시서 §3) ────────────────────────────────── */
{
  ...
  const st = await call("GET", { query: { ids: id } });
...
{
  _resetMem();
  const c = await call("POST", { body: { axes: AXES, name: "A" } });
  const nid = c.body.id;
  ...
  const st = await call("GET", { query: { ids: nid } });
...
/* ── ⑤ 생년월일 원값은 서버가 막는다 (처리방침 §5-2 「저장하지 않는 것」) ──── */
{
  for (const k of ["y", "birth", "YMD"]) {
    const bad = await call("POST", { body: { axes: { ...AXES, [k]: 1990 } } });
    ck(`⑤ axes 에 '${k}' 가 섞이면 거절한다`, bad.code === 400, `${bad.code}`);
  }
  const ok = await call("POST", { body: { axes: AXES } });
...
{
  _resetMem();
  const c = await call("POST", { body: { axes: AXES } });
  const did = c.body.id;
  ck("⑥ 취소 전엔 조회된다", (await call("GET", { query: { ids: did } })).body.length === 1);
  const d = await call("DELETE", { seg: [did] });
  ck("⑥ 취소가 성공한다", d.code === 200);
  ck("⑥ 취소하면 즉시 사라진다", (await call("GET", { query: { ids: did } })).body.length === 0);
...
/* ── ⑦ 만료 — 30일. 처리방침이 약속한 그 값이다 ──────────────────────────── */
{
  ...
  ck("⑦ 만료 전엔 있다", (await call("GET", { query: { ids: eid } })).body.length === 1);
  void mod;
}
...
/* ── ⑨ 출처 — 아무 데서나 부르지 못한다 ──────────────────────────────────── */
{
  const bad = await call("POST", { body: { axes: AXES }, origin: "https://evil.example" });
  ck("⑨ 허용 안 된 출처는 403", bad.code === 403, `${bad.code}`);
  const none = await call("POST", { body: { axes: AXES }, origin: "" });
...
/* ── ⑩ 조회 상한 — 곁 상한(24)과 맞춘다 ─────────────────────────────────── */
{
  _resetMem();
  const ids = [];
  for (let i = 0; i < 30; i++) ids.push((await call("POST", { body: { axes: AXES } })).body.id);
  const many = await call("GET", { query: { ids: ids.join(",") } });
