...
function lifeDomains(ctx) {
  ...
  /* 9. 사람 */
  {
    /* ⚠ 554 가 `"암록(暗祿)"` 로 push 한다 — 한자 없이 적으면 **영영 안 걸린다**(2026-08-29 수정).
       같은 화면 아래쪽은 `SIN_KO` 로 매핑돼 「숨은 복」이 뜨는데 위에서만 빠져 있었다. */
    const gw = sins.filter((x) => ["천을귀인", "암록(暗祿)"].includes(x.name));
...
void main(){
  ...
  /* ⚠ **평행이동은 「돌았다」가 아니라 「미끄러졌다」로 읽힌다**(창업자 2026-08-30:
     "얼굴만 돌고 몸은 안 도는 느낌"). 도는 물체는 **겉(실루엣)이 그대로 있고 속이 움직인다.**
     겉까지 같이 옮기니 회전 단서가 통째로 지워졌다. 여기서는 안 옮기고,
     아래 **안쪽 층(c0·c1)만** 고개 쪽으로 보낸다 — 그게 「앞면이 이쪽을 향했다」이다. */
  vec2 w = e;
  ...
  /* 안쪽 두 층이 **머리의 앞면**이다. 고개가 도는 쪽으로 이 둘만 간다 —
     겉은 제자리이므로 「실루엣은 그대로인데 앞면이 돌아섰다」가 된다. 그게 회전이다. */
  /* ⚠ 여기서 **스프링으로 부푼 R 을 쓰면 안 된다.** 확대 순간 R 이 1.37배로 치솟아
     앞면이 크게 튕겨 나가고, 전이 중 실루엣이 찌그러진다(검사 ⑩-b 가 0.288 로 물었다).
     회전은 크기와 무관하다 — 오버슈트가 없는 **목표 크기**로 잡는다. */
  float Rs = mix(0.320, 0.208, clamp(u_fold, 0.0, 1.0)) * mix(1.0, 0.20, tk);
  ...
  fwd += vec2(u_press.x, -u_press.y) * u_press.z * 0.032 * 2.35;
  ...
  float volA = 1.0;
  {
    ...
    /* ⚠ **음영을 덩어리 전체에 칠하면 안 된다**(실기 영상 2026-08-30).
       헤일로는 옅게 퍼진 빛이라 거기까지 어둡게 칠하니 **몸에서 떨어져 나온
       거대한 회색 얼룩**이 됐다 — 그림자가 아니라 딴 물체로 보인다.
       음영은 **살이 있는 곳**에만 준다. 바깥으로 갈수록 빠르게 사라지게 한다. */
    float dens = 1.0 - smoothstep(0.34, 0.88, q);
    float vol = mix(0.26, 1.10, smoothstep(0.10, 0.94, lam));
    outc *= mix(1.0, vol, dens);
...
function GuardianField({ saju, mood, orbRef, reactRef, scatter, size = 340, onFail }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* ⚠ **몸의 가로 폭은 오행마다 다르다**(셰이더의 agp.x — 클수록 가로가 좁다).
         눈 간격을 한 값으로 두면 화·목처럼 **가늘고 긴 몸에서는 눈이 밖으로 나간다**
        ...
        if (FACE && faceRef.current) {
          ...
          if (k > 0.05) {
            ...
            /* ⚠ 예전엔 드리프트+위습만 썼다 — 그래서 **코어에서 떠 있었다**(실측 x 28px·y 22px).
               ...
               눌리면 그 위에 밀린 만큼을 더한다. */
            /* ⚠ 0.052 는 **몸이 좁을 때 얼굴을 몸 밖으로 밀어냈다**(실기 영상 2026-08-30 —
               드래그 중 프레임에서 눈이 배경 위에 있었다). 로컬 명식에서는 재현이 안 됐고
               영상 속 체형이 더 가늘다. 재현 못 한 채로 값을 그대로 두는 건 방치이므로 줄인다.
               ⚠ 셰이더의 심지 이동(u_press 항)과 **같은 값**이어야 한다. */
            const cxr = core.x + pdx * 0.032 * press;
            const cyr = core.y - pdy * 0.032 * press;
            ...
            drawFace(g2, S, {
              ...
              /* ⚠ **곁 구를 상수(0.216)로 부풀리면 안 된다.** 곁의 몸은 응축돼 실측 코어가
                 40px 인데 그 상수는 127px 짜리 구를 만든다 — 눈이 몸 밖으로 나간다
                 (실기 확인: 곁에서 눈·홍조가 배경 위에 떴다. 눈/코어 비 1.55).
                 「곁 얼굴 원복」은 살리되 **몸에 붙인 채로** 키운다 — 코어를 따라가되 곁에서 +35%.
                 곁 RAD ≈ 70px, 눈 ±45px, 구슬 반경 ≈ 72px 로 **간신히 안쪽**이다. */
              radFrac: (coreR / 2.35) * (1 + 0.35 * fvFace),
              /* ⚠ 0.56 은 **몸이 좁을 때 눈이 밖으로 나갔다**(실기 영상 2026-08-30 — 오른눈이
                 배경 위에 떠 있었다). 몸은 오행마다 모양이 다르고 불꽃형은 폭이 좁다.
                 구의 0.42 자리로 좁힌다 — 좁은 몸에서도 살 안에 들어온다. */
              /* ⚠ 보드에서 고른 값은 **최종 gapR 0.64**(창업자 2026-08-30)다. 여기 상수는
                 오행 폭으로 나누기 **전** 값이므로 기본 오행(금 1.08) 기준으로 되돌려 잡는다 —
                 0.64 × 1.08 = 0.691. 그래야 금에서 정확히 0.64 가 되고, 좁은 오행에서는
                 그만큼 좁아진다(화 0.55 · 목 0.59). */
              gapR: (0.691 / AGX) * k * (1 - 0.42 * press),
...
5) "무엇을 걸고 — 이 판결이 틀릴 조건까지" — 제목을 **이 줄 그대로** 쓴다(부제까지). 두 가지를 반드시 담는다. ①${cost}을 하나, 정직하게 명시한다(좋은 말만 하지 않는다). ②마지막 줄에 **반증 조건**: "이런 일이 벌어지면 이 판결을 뒤집어라". 조건은 감정이 아니라 **눈으로 확인되는 사건**이어야 한다.
...
바꿔 쓰는 말(이어서): 명식=네 여덟 글자 · 십성=여덟 글자끼리 맺는 관계 · 일간=너 자신을 나타내는 글자 ·
일지=그 글자가 앉은 자리 · 배우자궁=배우자 자리 · 신살=곁따르는 기운 · 지장간=글자 속에 숨은 기운 ·
억부=힘의 균형 · 조후=춥고 더운 균형 · 만세력=날짜를 글자로 옮기는 표 · 진태양시=해를 기준으로 고친 시각 ·
유파=보는 방식 · 통설=널리 쓰는 풀이
  ...
  return (
      ...
      {step === 0 && (
        <section className="scene fade">
          ...
          <p className="ainote">사주 계산(만세력)은 자동검증 28문항을 통과한 엔진이 해요 ·
            질문과 답변은 <b>이름·연락처를 지운 뒤</b> 품질 확인용으로만 기록해요 — 수호신이 생기면 <b>홈 맨 아래 「사용 통계 수집을 끌래」</b>로 끌 수 있어요</p>
            {/* ⚠ **「아래에서」로 쓰지 마라 (2026-08-29 정정).** 이 화면(step 0)에는 그 스위치가 없다.
                실물은 7494 인데 `phase>=1 && !res && awake` 안이라 온보딩 5칸 + 형성 3.2초 + **더블탭** 뒤에야 뜬다.
                같은 파일 6980 은 처음부터 「홈 화면 아래쪽 「사용 통계 수집을 끌래」」로 정확히 적고 있었다 —
                A-1 사고를 고칠 때 동의문만 고치고 이 줄이 남았다. 문구는 **버튼 이름 그대로** 적는다. */}
        </section>
      )}
    ...
    남는 길은 **그 사람에게 링크를 보내는 것뿐**이다. 화면이 실제로 되는 일만 말해야 한다. */}
                ...
                {/* ⚠ **여기서 「서버로도 안 나가」로 끝내면 거짓이 된다 (2026-08-29 정정).**
                    ...
                    곁에 적어 둔 **상대들** 이름은 정말로 기기에만 있다 — 그 둘을 갈라 적는다.
                    같은 절 6580 주석이 이 사고를 이미 이름 붙여 놨다: 약속을 먼저 쓰고 코드가 안 따라간 자리.
                    ⚠ 문구만 고치면 반쪽이다 — 처리방침 §5-2 「저장하는 것」에도 같이 적었다. */}
          ...
          {phase >= 1 && !res && !awake && (
            <div className="lobbypanel fade">
              ...
              {/* ⚠ **깨우는 건 더블탭이다** — `tryWake` 가 `now - wakeTapRef < 350` 일 때만 `setAwake(true)` 를 부르고
                  ...
                  **전 유저가 매 방문 반드시 통과하는 이 문에서만** 「한 번」이었다(2026-08-29 정정). */}
              <p className="wakehint">{letterSent ? "두 번 두드려봐 — 하나 더 물어도 돼" : "두 번 두드려봐 — 답은 거기 있어"}</p>
            </div>
          )}
