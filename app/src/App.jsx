...
function DustOrb({ size = 160, stage = 0, tint }) {
  ...
  useEffect(() => {
    ...
    const ctx = cv.getContext("2d");
    const w = size, cx = w / 2, cy = w / 2, R = w * 0.42;
    const grow = Math.min(3, Math.max(0, stage));               // v23: 조각을 바칠 때마다 자란다
    const [tc1, tc2] = tint || [];
    const ps = Array.from({ length: 150 + grow * 140 }, (_, i) => {
      const a = Math.random() * Math.PI * 2, r = Math.pow(Math.random(), 0.7) * R * (0.8 + grow * 0.07);
      return { a, r, s: Math.random(), o: Math.random() * 100, tinted: tint ? i % 3 === 0 : false };
    });
    let t = 0, raf;
    ...
    const draw = () => {
      ...
      const coreR = R * (0.58 + grow * 0.09) * b1;
      const g1 = ctx.createRadialGradient(cx, cy, 1, cx, cy, coreR);
      g1.addColorStop(0, (tc1 || "#ffe9ad") + "3a"); g1.addColorStop(1, "transparent");
      ...
      ps.forEach(p => {
        ...
        ctx.globalAlpha = (0.4 + 0.6 * Math.sin(t * 4 + p.o * 6)) * (0.4 + p.s * 0.5) * (0.75 + grow * 0.14);
        ctx.fillStyle = p.tinted && tc2 ? tc2 : (p.o % 3 < 1 ? "#ffe9ad" : "#cdd6ff");
        ctx.beginPath(); ctx.arc(x, y, 0.5 + p.s + grow * 0.12, 0, 7); ctx.fill();
      });
      ctx.globalAlpha = 1;
      raf = requestAnimationFrame(draw);
    };
...
## 층위·가중치
기질 층(MBTI·별자리·수비학 라이프패스·가치[요즘]·달[달 별자리·나크샤트라=정서와 본능]·마야 문양) / 타이밍 층(사주 오행·달 위상·바이오리듬[오늘]·삼재[해당 연도만]·주역 괘[유저가 동전으로 청한 경우만]). A: 기질50/타이밍50, B: 타이밍55/기질45, C: 타이밍만. 혈액형: 판결 미반영, 곁의 '정령'이 거드는 재미 한마디 전용.
## 3화법
...
- reasons에는 판결에 참여한 모든 지표를 각 1줄씩 빠짐없이 포함한다(혈액형 제외) — 사주·달·별자리·MBTI·수비학·바이오리듬·마야와, 제공된 경우 삼재·가치·주역·토정비결까지 전부. 달 축은 위상·달 별자리·나크샤트라를 묶어 한 줄로, 사주 축은 납음을 함께 인용할 수 있다. 각 축이 왜 GO/STOP/중립인지 그 지표의 실제 값을 짚어서 말한다.
...
## 출력(JSON만, 백틱·서문 금지)
{"category":"A|B|C","tone":"단호|격려|충고","verdict":"한 문장 단답","subline":"수호신의 한 줄","against":숫자,"total":숫자,"direction":"GO|STOP|HOLD","reasons":[{"axis":"사주|달|별자리|MBTI|수비학|주역|가치|바이오리듬|삼재|토정비결|마야","vote":"GO|STOP|중립","text":"회상체 근거 1줄(60자 이내)"}],"funLine":"정령(혈액형) 한마디","disclaimer":"해당 시에만, 없으면 빈 문자열"}`;
...
function loadMemory() {
  try {
    ...
    return m && m.saju && m.mbti && m.blood && m.core ? m : null;   // 필수 조각 검증 — 손상 시 새 출발
  } catch (_) { return null; }
}
...
export default function App() {
  ...
  const [blood, setBlood] = useState(mem?.blood || null);
  ...
  useEffect(() => {
    if (step === 3 && saju && mbti && blood && core) {
      saveMemory({ birth, saju, zo, moon, num, mbti, blood, vals8, vals4, core, convo, records, streak });
    }
  }, [step, saju, mbti, blood, core, convo, records, streak]);
  ...
  const fetchDetail = async (system, priorConvo, userText, r1) => {
    setDetailBusy(true);
    try {
      const explainMsg = { role: "user", content: `${userText}\n\n[이미 확정된 판결] direction=${r1.direction} / verdict="${r1.verdict}" / 총 ${r1.total} 중 반대 ${r1.against}. 이 판결을 절대 뒤집지 말고, 이 결론의 근거만 아래 JSON으로만 응답: {"subline":"수호신의 한 줄","reasons":[{"axis":"사주|달|별자리|MBTI|수비학|주역|가치|바이오리듬|삼재|토정비결|마야","vote":"GO|STOP|중립","text":"회상체 근거 1줄(60자 이내)"}],"funLine":"정령(혈액형) 한마디","disclaimer":"투자·의료·법률일 때만, 없으면 빈 문자열"}. reasons엔 판결에 참여한 지표 전부(혈액형 제외).` };
  ...
  return (
    <div className="stage">
      ...
      {step === 2 && saju && (
        <section className="scene fade">
          <div className="halo">
            <DustOrb size={210} stage={mbti && blood ? 2 : 1} tint={saju ? EL_COLOR[saju.main] : undefined} />
            <div className="gtext">
          ...
          {reveal >= 5 && (
            <div className="fade">
              ...
              <p className="sub2 mt">요즘의 너는 어떤 모습이야? — 내 기억이 맞는지 골라줘.</p>
              <div className="dims">
                {[
                  ["EI", "E", "사람들 속에서 기운을 얻는 쪽", "I", "혼자일 때 차오르는 쪽"],
                  ["SN", "N", "아직 오지 않은 것을 보는 쪽", "S", "눈앞의 확실한 것을 보는 쪽"],
                  ["TF", "T", "머리가 먼저 정리하는 쪽", "F", "마음이 먼저 움직이는 쪽"],
                  ["JP", "J", "정해둔 길이 편한 쪽", "P", "열어둔 길이 편한 쪽"],
                ].map(([k, a, at, b2, bt]) => (
                  <div key={k} className="dimrow">
                    <button className={`dimopt ${dims[k] === a ? "sel" : ""}`} onClick={() => pickDim(k, a)}>{at}</button>
                    <button className={`dimopt ${dims[k] === b2 ? "sel" : ""}`} onClick={() => pickDim(k, b2)}>{bt}</button>
                  </div>
                ))}
              </div>
          ...
          {phase >= 1 && !res && (
            <div className="fade gpanel">
              ...
              {!ritual && returning && !res && (
                <button className="resetlink" onClick={() => { if (window.confirm("정말 처음부터 할까? 지금의 수호신과 기억이 흩어져.")) { clearMemory(); window.location.reload(); } }}>다른 사람이야? — 처음부터 다시</button>
              )}
              {ritual && !res && (
...
.dims{display:flex;flex-direction:column;gap:8px;width:100%}
.dimrow{display:flex;gap:8px}
.dimopt{flex:1;font-family:inherit;font-size:12.5px;padding:12px 8px;border-radius:14px;border:1px solid rgba(138,127,149,.35);background:transparent;color:#9d8fb5;cursor:pointer;transition:all .25s;line-height:1.5;word-break:keep-all}
