...
- **판정 절차(출력 순서로 강제된다 — 최중요)**: ①votes를 **먼저** 쓴다. 각 지표를 질문에 비추어 서로 독립적으로 GO/STOP/중립 판정한다 ②그 표를 세어 direction을 정한다(**많은 쪽 그대로**. 동률이면 표가 갈린 것이니 HOLD — '해보는 쪽'으로 밀지 마라. 그건 지표가 아니라 인생관이다) ③verdict는 **이미 정해진 direction을 말로 옮긴 것**이다.
...
function tallyVotes(r1) {
  ...
  /* v132.3: 동률이면 GO 였다 — '해보는 쪽이 인생에 남는다'는 경험 편향을 코드로 굳힌 자리다.
     그건 지표가 아니라 인생관이고, 표가 반반인데 한쪽으로 미는 건 표를 안 따르는 것이다.
     이제 동률은 동률로 둔다(HOLD = 갈렸다). 실측상 바뀌는 판결은 없다 — 60일 77건 중 동률 0건. */
  const dir = modelDir === "HOLD" ? "HOLD" : go === stop ? "HOLD" : go > stop ? "GO" : "STOP";
