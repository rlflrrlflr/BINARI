...
const THICK = {
  식상: { job: "만들어서 내놓는 쪽", ex: "기획·창작·교육·요식처럼 결과물이 네 이름으로 나가는 일", grew: "시키는 대로 하는 게 답답했고, 네 방식으로 바꿔야 손이 움직였어" },
  재성: { job: "굴려서 남기는 쪽", ex: "영업·유통·중개·자영업처럼 거래가 곧 실력인 일", grew: "값과 이익이 먼저 보였고, 숫자로 말할 때 설득력이 붙었어" },
  관성: { job: "조직 안에서 자리를 받는 쪽", ex: "공공·법률·행정·교육·인사처럼 규칙을 다루고 오래 앉아 있는 일", grew: "규칙이 분명한 데서 오히려 편했고, 맡기면 끝까지 했어" },
  인성: { job: "배워서 푸는 쪽", ex: "연구·상담·자격이 필요한 전문직처럼 아는 만큼 값이 붙는 일", grew: "설명해 주면 빨리 알아들었고, 다 알고 나서야 움직였어" },
  비겁: { job: "제 힘으로 미는 쪽", ex: "1인 사업·프리랜서·기술직처럼 실력이 곧 간판인 일", grew: "누구 밑보다 혼자가 빨랐고, 그래서 부딪히기도 했어" },
};
const ORGAN = {
...
export function readImprint({ saju, ladder, birth, sex, now = new Date(), lat = SEOUL.lat, lon = SEOUL.lon,
  ...
  putD("공부", "공부 — 어떻게 붙나",
    ...
    nowLine("인성", "배움이 붙는 열 해야 — 학교·자격·문서가 유독 잘 풀려", "배움이 저절로 붙는 때는 아니야. 길게 잡지 말고 짧게 끊어서 필요한 것만 사 와"),
    nextLine("인성", "여든까지 그 자리가 열리는 열 해는 없어 — <b>배움은 필요할 때 직접 사둬야 해</b>"),
    fn(`배움을 맡은 자리 ${G.인성}개.`));

  putD("일", "일 — 어디서 밥을 버나",
    `<b>${THICK[thickKey].job}</b>으로 새겨졌어.`,
    `${dot(THICK[thickKey].grew)} 맞는 판은 <b>${THICK[thickKey].ex}</b>${jong(THICK[thickKey].ex) ? "이야" : "야"}.`,
    nowLine(thickKey, "네 방식이 그대로 먹히는 열 해야. 판을 바꾸려면 지금이야", "네 결이 아닌 쪽이 힘을 쓰는 때야. 억지로 미는 것보다 배우는 데 쓰는 게 남아"),
...

  {
    ...
    putD("돈", "돈 — 얼마나, 언제",
      ...
      nowLine("재성", "돈이 실제로 움직이는 열 해야 — 계약·거래·목돈이 이 구간에 몰려", "버는 것보다 지키는 쪽에 서 있어. 무리해서 굴리면 새"),
  ...
  putD("사람", "사람 — 곁에 누가 서나",
    ...
    "먼저 다가가는 일은 드물었을 거야. 그래서 손해 보는 자리가 딱 여기야 — <b>부탁을 못 해서 혼자 다 져.</b>",
    nowLine("비겁", "사람이 몰리는 열 해야 — 동업·팀·독립 이야기가 이 구간에 나와", "사람보다 네 일이 먼저인 때야. 사람은 그 다음에 따라와"),
    ...
    putD("연애", "연애 — 인연이 오는 방식",
      ...
      married === true
        ? (cur && GRP_OF[cur.ss] === g2
            ? `<b>${cur.from}~${cur.to}세 — 지금이 그 자리가 움직이는 열 해야.</b> 이미 있는 사이라면 <b>이때 온도가 크게 바뀌어</b> — 좋은 쪽으로든 아닌 쪽으로든`
            : "지금은 그 자리가 조용해. 관계가 밋밋하게 느껴진다면 사이가 식은 게 아니라 <b>흐름이 다른 데 가 있는 거야</b>")
        : nowLine(g2, "인연이 실제로 움직이는 열 해야", "저절로 굴러오는 때는 아니야. 오면 네가 만든 자리에서 와"),
  ...
  const checks = [
    ...
    [cur ? `요즘이 「${cur.title}」 같은가` : "요즘 흐름이 바뀌는 느낌이 있는가", "지금 열 해로 적었어"],
  ];
