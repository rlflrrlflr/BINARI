...
import {
  ...
  nayin, honmeisei, weton, akan, lifePath, moonPhase, partOfFortune, HOUSE_KO, JI_EL, midheaven,
} from "./sky.js";
...
const BLOCK = {
  식상: { burn: "회의에서 말을 안 하게 돼. 의견이 없어서가 아니라 <b>꺼내는 게 귀찮아진 것</b>이야", t: "말", plain: "하고 싶은 말을 제때 못 꺼내",
  ...
  재성: { burn: "성과 얘기가 나오면 피하게 돼. <b>숫자를 안 보기 시작하면</b> 그게 신호야", t: "돈 쥐기", plain: "돈이 손에 안 남아",
    s: "지나가는 건 보이는데 남는 게 없어", w: "크게 벌어도 남는 게 적어. 버는 재주보다 지키는 장치가 먼저야",
    trig: "돈이 계속 새 나갈 때",
    fix: "<b>월급이나 고정 계약으로 받아.</b> 투자·사업처럼 네가 직접 굴리는 돈은 새. 굴릴 거면 남한테 맡겨" },
  관성: { burn: "출근이 늦어지고 보고가 밀려. <b>규칙부터 무너지는 게</b> 네 신호야", t: "규칙", plain: "네 생활을 잡아 줄 틀이 없어",
  ...
  인성: { burn: "도와달라는 말을 더 안 하게 돼. <b>혼자 남는 야근이 늘면</b> 이미 진행 중이야", t: "도움받기", plain: "남한테 부탁을 못 해",
    s: "혼자 다 지고 가", w: "도와줄 사람이 없어서가 아니라 안 불러서야. 그래서 늘 네가 제일 지쳐",
    trig: "혼자 다 지려고 할 때",
    fix: "<b>\"도와줘\"라고 먼저 말하는 것.</b> 딱 이 한 마디야. 네가 평생 배워야 할 기술은 이거 하나뿐이야" },
  비겁: { burn: "사람을 피하게 돼. <b>점심을 혼자 먹기 시작하면</b> 그게 신호야", t: "사람", plain: "무리에 잘 못 섞여",
...
/* ── 직장생활 표 ────────────────────────────────────────────────────────
   ...
   「일 — 어떤 일이 맞나」 한 칸으로는 얕았다. 직장은 **직업 선택과 다른 축**이다 —
   무슨 일을 하느냐가 아니라 **조직 안에서 어떻게 굴러가느냐**다. 명리 직업론이 실제로 그렇게 갈린다:
   관성 = 조직·직함·상사, 비겁 = 동료·독립, 식상 = 내 방식, 재성 = 성과, 인성 = 배움·문서. */
const WORK_STYLE = {
  목: { w: "일을 벌이는 쪽", d: "새 프로젝트가 재밌고 마무리가 지루해. 시작은 네가 하고 끝은 남이 내는 그림이 반복돼" },
  화: { w: "속도로 미는 쪽", d: "빨리 해치우고 다음으로 가. 대신 급하게 나간 게 나중에 손볼 거리로 돌아와" },
  토: { w: "안 무너지게 받치는 쪽", d: "화려하진 않은데 네가 빠지면 티가 나. 대신 티가 안 나서 평가에서 밀려" },
  금: { w: "기준으로 자르는 쪽", d: "아닌 건 아니라고 해서 일이 깔끔해져. 대신 사람 사이에서 각이 서" },
  수: { w: "재 보고 움직이는 쪽", d: "리스크를 먼저 봐서 사고가 적어. 대신 결정이 늦어 기회를 놓쳐" },
};
const WORK_GOOD = {
  식상: "없던 걸 만들어 내는 일 — 기획·설계·콘텐츠. 흰 종이에서 시작하는 걸 남보다 잘해",
  재성: "숫자와 거래를 다루는 일 — 영업·제휴·손익. 값을 매기고 남는 걸 계산하는 감이 있어",
  관성: "판을 관리하는 일 — 운영·일정·조율. 여러 갈래를 안 흘리고 끌고 가는 데 강해",
  인성: "파고들어 정리하는 일 — 분석·조사·문서. 남들이 대충 넘기는 데서 근거를 찾아내",
  비겁: "혼자 끝까지 붙드는 일 — 실무·기술·전담. 맡기면 확인 안 해도 되는 사람이야",
};
const WORK_BAD = {
  식상: "네 방식대로 못 하면 손이 안 움직여. 남이 짜 놓은 틀에 채워 넣는 일에서 제일 지쳐",
  재성: "숫자로 성과를 증명해야 하는 자리에서 밀려. 잘해 놓고도 설명을 못 해",
  관성: "여러 갈래를 동시에 굴리는 걸 못 견뎌. 일정·보고 같은 관리 업무에서 구멍이 나",
  인성: "근거 없이 밀어붙이는 자리에서 불안해져. 준비가 덜 되면 시작을 못 해",
  비겁: "혼자 하려다 병목이 돼. 나눠 주면 훨씬 빠른데 그걸 못 해",
};
const BOSS = {
  정관: { g: "규칙이 분명하고 예측 가능한 상사", b: "기분으로 움직이는 상사 — 기준이 매일 바뀌면 네가 먼저 무너져" },
  편관: { g: "몰아붙이지만 책임은 지는 상사", b: "결정을 미루는 상사 — 답을 안 주면 네가 대신 지게 돼" },
  없음: { g: "네게 판을 맡기고 안 들여다보는 상사", b: "하나하나 확인하는 상사 — 감시받는다고 느끼면 성과가 반으로 떨어져" },
};
const TENURE = {
  안정: { w: "한 곳에 오래", d: "5년 이상이 자연스러워. 옮기는 걸 스스로 잘 못 정해서, 밀려서 옮기는 쪽이야" },
  보통: { w: "3~5년 주기", d: "한 사이클 돌리고 다음을 보는 편이야. 배울 게 떨어지면 몸이 먼저 알아" },
  이동: { w: "2~3년 주기", d: "오래 못 앉아 있어. 문제는 지루함이 아니라 <b>다음을 정하기 전에 나오는 것</b>이야" },
};
const POSITION = {
  실무: "실무의 정점까지야. 사람을 관리하는 자리로 가면 오히려 값이 떨어져 — 네 값은 네가 직접 하는 데서 나와",
  중간: "중간에서 위아래를 잇는 자리가 맞아. 위로도 아래로도 말이 통하는 몇 안 되는 자리야",
  리더: "결정하는 자리까지 가. 다만 네가 다 하려 들면 팀이 안 커 — 맡기는 게 네 마지막 관문이야",
  독립: "조직 안에서 끝까지 가는 그림이 아니야. 언젠가 네 이름으로 하게 돼 — 그게 빠르냐 늦냐의 문제야",
};
/* 중천(MC) 사인 → 사회에서 네가 서는 자리. 서양 전통에서 MC 는 직업·명예의 축이다 */
const MC_FIELD = {
  양자리: "먼저 뛰어드는 자리 — 신규·개척·경쟁이 있는 판", 황소자리: "값을 다루는 자리 — 금융·부동산·제조·먹는 것",
  쌍둥이자리: "말과 정보를 옮기는 자리 — 미디어·교육·영업·중개", 게자리: "돌보는 자리 — 의료·복지·식음·고객",
  사자자리: "무대 앞에 서는 자리 — 브랜드·엔터·대표성이 있는 역할", 처녀자리: "정밀한 자리 — 분석·품질·회계·의료기술",
  천칭자리: "사이를 맞추는 자리 — 법률·인사·디자인·파트너십", 전갈자리: "깊이 파는 자리 — 조사·보안·심리·구조조정",
  사수자리: "멀리 가는 자리 — 무역·해외·학문·출판", 염소자리: "체계를 세우는 자리 — 공공·대기업·인프라",
  물병자리: "판을 뒤집는 자리 — 기술·스타트업·비영리", 물고기자리: "보이지 않는 걸 다루는 자리 — 예술·치유·연구",
};
...
export function readImprint({ saju, ladder, birth, sex, now = new Date(), lat = SEOUL.lat, lon = SEOUL.lon,
  ...
  putD("몸", "몸 — 어디가 약하게 왔나",
    ...
    (() => { const f = bands.find((b) => b.from > age && b.el === weakEl[0]);
      return f ? (emptySide ? `<b>${f.from}~${f.to}세</b>부터 그 기운이 들어와 — 그 열 해에 몸이 한 단계 편해져`
        : `<b>${f.from}~${f.to}세</b>에 그 기운이 또 들어와 — 이미 넘치는 쪽이라 <b>그 구간엔 특히 관리가 필요해</b>. 무리한 일정과 늦은 밤을 줄이고, 정기 검진을 그때로 잡아 둬`)
    ...
    fn(`축 ${axes.length}개 — ${axes.map((a) => a.why).join(" / ")}. ${healthAgree ? `${weakEl[0]}이 겹쳐 한 곳으로 봤다` : "겹치는 자리가 없어 전부 적었다"}.
      <b>근거의 급을 밝힌다</b>: 오행-장부 대응(목=간·담, 화=심장, 토=비위, 금=폐·대장, 수=신장·방광)은 한의학 전통의 배당 그대로라 <b>변환이 없다</b>. 다만 그 배당을 <b>개인의 건강 예측으로 옮기는 데는 해석이 섞이고, 우리는 이 판정이 맞았는지 검증한 적이 없다</b>.
      <b>이건 의료 판단이 아니다</b> — 증상이 있으면 병원에 가는 것이 먼저다.`));
  ...
  /* ── 직장생활 ────────────────────────────────────────────────────────
     「일 — 어떤 일이 맞나」는 **직업 선택**의 축이고, 이 절은 **조직 안에서 어떻게 굴러가나**의 축이다.
     둘은 다르다 — 맞는 직업을 골라도 조직에서 못 버티는 사람이 있다.
     이직 시기는 두 층으로 짚는다: 대운(십 년 판)과 세운(올해·내년). 세운은 실제 계산에서 나온다. */
  const work = (() => {
    if (!bands.length && G.관성 === 0 && G.비겁 === 0) return null;
    const gwan = G.관성, bi = G.비겁, sik = G.식상;
    const posKey = bi >= 3 && gwan <= 1 ? "독립" : gwan >= 3 ? "리더" : gwan === 0 ? "실무" : bi >= 2 ? "중간" : "중간";
    const tenKey = gwan >= 2 && bi <= 1 ? "안정" : (bi >= 3 || sik >= 3) ? "이동" : "보통";
    const bossKey = ss.정관 ? "정관" : ss.편관 ? "편관" : "없음";
    /* 같이 일하면 좋은 사람 — 나를 낳아 주는 오행(인성 방향)과, 내가 낳는 오행(식상 방향) */
    const feeder = { 목: "수", 화: "목", 토: "화", 금: "토", 수: "금" }[me];
    const drain = SANG[me];
    /* 이직 — ① 십 년 판이 바뀌는 해 ② 올해부터 여섯 해 중 움직이기 좋은 해·나쁜 해 */
    const nextBand = bands.find((b) => b.from > age) || null;
    const push = bands.filter((b) => ["편관", "겁재"].includes(b.ss) && b.to >= age).slice(0, 1)[0] || null;
    const yearRows = yrs.map((y) => {
      const t = ssOf(idx.dG, gIdx(y.ganji[0]));
      return { year: y.year, f: y.f, ss: t,
        w: y.f > 0 && ["정관", "편관", "정인"].includes(t) ? "옮기기 좋은 해 — 자리가 열려"
          : y.f > 0 ? "지금 자리에서 값이 오르는 해"
            : y.f < 0 && ["편관", "겁재"].includes(t) ? "밀려서 움직이게 되는 해 — 미리 준비해 둬"
              : y.f < 0 ? "움직이면 손해 보는 해 — 버티는 게 남아"
                : "크게 흔들리지 않는 해" };
    });
    const goYear = yearRows.find((y) => /옮기기 좋은/.test(y.w)) || null;
    const badYear = yearRows.find((y) => /움직이면 손해|밀려서/.test(y.w)) || null;
    const mcSign = signOf(midheaven(jd, lon));
    return {
      rows: [
        ["일 스타일", `<b>${WORK_STYLE[me].w}</b>이야. ${dot(WORK_STYLE[me].d)}`,
          fn(`일간 ${GANK[idx.dG]}(${me})의 업무 성정.`)],
        ["잘하는 것", WORK_GOOD[thickKey], fn(`가장 두꺼운 자리 ${thickKey} ${G[thickKey]}개 — 이 십성이 맡는 업무 영역.`)],
        ["안 되는 것", WORK_BAD[blockKey], fn(`가장 빈 자리 ${blockKey} ${G[blockKey]}개. <b>못하는 걸 아는 게 잘하는 걸 아는 것보다 쓸모가 크다</b> — 피할 수 있으니까.`)],
        ["어느 위치까지", POSITION[posKey], fn(`조직의 틀을 맡는 자리 ${gwan}개 · 혼자 미는 자리 ${bi}개 → ${posKey}.`)],
        ...
        ["서양이 보는 네 자리", `<b>${MC_FIELD[mcSign] || "특별히 치우친 분야는 안 잡혀"}</b>${MC_FIELD[mcSign] ? (jong(MC_FIELD[mcSign]) ? "이야" : "야") : ""}. 사주가 <b>어떻게 일하나</b>를 말한다면 이건 <b>어디에서 이름이 나나</b>를 말해 — 축이 달라서 겹치지 않아.`,
          fn(`중천(MC) ${mcSign}. 서양 전통에서 MC 는 직업·사회적 정점의 축이다. 분야로 옮기는 데 <b>해석이 섞인다</b>.`)],
        ["번아웃 신호", `<b>${dot(BLOCK[blockKey].burn)}</b> 그 다음은 몸이야 — ${dot(ORGAN[weakEl[0]].sym.split(",")[0])} <b>여기까지 오면 늦어. 앞 신호에서 끊어.</b>`,
          fn(`가장 빈 자리 ${blockKey}이 직장에서 드러나는 형태 → 가장 약한 자리 ${weakEl[0]}의 증상. <b>압력은 빈 자리에서 시작해 얇은 자리로 간다</b> — 순서를 알면 앞에서 끊을 수 있다.`)],
      ],
      years: yearRows, goYear, badYear,
      moveBand: push ? { from: push.from, to: push.to, why: push.ss === "편관" ? "책임이 갑자기 얹히면서" : "겨루는 일이 생기면서" } : null,
      nextBand: nextBand ? { from: nextBand.from, to: nextBand.to, title: nextBand.title } : null,
      n: fn(`직장 축은 관성(조직·상사)·비겁(동료·독립)·식상(내 방식)의 배치로 본다. 이직 시기는 대운(십 년 판)과 세운(해마다) 두 층으로 짚었고, 세운 순역은 채울 기운(${yong.join("·")}) 대비로 계산했다.`),
    };
  })();
  ...
  return {
    ...
    domains: D, checks, mateMode, sky9, clash, money, compass, saga, work,
