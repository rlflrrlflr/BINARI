...
export function readMatch({ a, b, lat = 37.5665, lon = 126.978 } = {}) {
  ...
  const work = (() => {
    ...
    const ROLE = {
      비견: ["대등한 동료", "둘 다 같은 자리에 서. 나눠 맡으면 잘 굴러가는데 <b>같은 걸 서로 하겠다고 할 때</b> 부딪혀 — 시작할 때 담당을 글로 갈라 둬"],
      겁재: ["같은 걸 노리는 사이", "속도는 붙는데 <b>몫을 나눌 때 말이 갈려.</b> 성과·지분·이름 순서를 <b>일 시작 전에</b> 정해 두면 대부분 안 터져"],
      식신: ["네가 판을 만들고 상대가 굴리는 사이", "네 아이디어를 상대가 실물로 만들어. 편한 조합인데 <b>공이 상대에게 가 보인다</b> — 밖에 알릴 때 네 이름도 같이 걸어"],
      상관: ["네가 밀고 상대가 튀는 사이", "재밌는 게 나오는 조합이야. 대신 <b>상대가 네 방식에 토를 자주 달아</b> — 그게 실력이지 반항이 아니라는 걸 네가 먼저 인정해야 굴러가"],
      정재: ["네가 관리하는 쪽", "숫자·일정이 네 손에 있으면 안정돼. <b>상대가 답답해할 수 있어</b> — 재량 범위를 미리 떼어 줘"],
      편재: ["같이 크게 벌이는 사이", "판을 키우는 데는 최고야. 대신 <b>둘 다 관리를 미뤄</b> — 돈·서류를 볼 세 번째 사람이 필요해"],
      정관: ["상대가 틀을 잡는 쪽", "상대가 기준을 세우면 네가 그 안에서 잘 움직여. <b>상하 관계에서 특히 편해</b> — 대등한 동업이면 네가 답답해질 수 있어"],
      편관: ["상대가 몰아붙이는 쪽", "성과는 빨리 나. 대신 <b>네가 소모돼</b> — 마감·연락 시간에 선을 그어 두지 않으면 오래 못 가"],
      정인: ["상대가 받쳐 주는 쪽", "상대가 네 뒤를 봐 줘서 네가 멀리 갈 수 있어. <b>결정은 네가 내려야 해</b> — 미루면 상대가 대신 내리고 나중에 네가 후회해"],
      편인: ["상대가 다른 길을 여는 쪽", "네가 못 본 각도를 상대가 봐. 대신 <b>일이 자꾸 옆으로 새</b> — 이번에 뭘 끝낼지 한 줄로 붙여 두고 시작해"],
    }[ss] || ["결이 잘 안 잡히는 사이", "일 축에서는 특별히 갈리는 게 안 잡혀. 방식보다 <b>무엇을 맡을지</b>부터 정하는 게 나아"];
