...
/* ═══════════════ 비나리 BINARI · MVP 통합 프로토타입 (v15: 2콜 판결·3층 리빌·HD 제거) ═══════════════
   ...
   보류: 토정비결·손없는날 근사 제외(음력 변환 라이브러리 필요) */
...
/* ───── v7 지표: 바이오리듬 · 삼재 · 가치 · 휴먼디자인 ───── */
const HD_TYPES = ["제너레이터", "매니페스팅 제너레이터", "프로젝터", "매니페스터", "리플렉터"];
const VALUES24 = ["안정", "성장", "자유", "인정", "관계", "성취", "즐거움", "의미", "돈", "건강", "배움", "용기", "정직", "창조", "평온", "모험", "가족", "영향력", "독립", "봉사", "아름다움", "전문성", "몰입", "유머"];
const HD_DESC = { "제너레이터": "세상의 다수 — 묻는 말에 몸이 먼저 반응하는 응답형. 끌리면 하고, 아니면 안 하는 게 맞는 사람.",
  "매니페스팅 제너레이터": "응답하면 누구보다 빠른 멀티트랙형. 여러 갈래를 동시에 달려도 되는 사람.",
  "프로젝터": "초대받을 때 빛나는 안내자형. 먼저 나서기보다 알아봐주는 곳에서 힘이 나.",
  "매니페스터": "먼저 치고 나가는 개시자형. 허락 말고 통보가 어울리는 사람.",
  "리플렉터": "주변을 비추는 거울형 — 가장 희귀해. 환경이 곧 컨디션인 사람." };
...
/* v15: Claude 호출 공용 헬퍼 — system(캐싱)+messages, 반환은 {json, txt} */
...
export default function App() {
  ...
  const [hd, setHd] = useState(null);           // v7: 휴먼디자인(자가 선택)
      ...
      {step === 0 && (
        <section className="scene fade">
          ...
          <div className="row gap">
            ...
            <button className="btn ghost" onClick={() => setStep(1)}>건너뛰기</button>
          </div>
        ...
        <section className="scene fade">
          ...
          {phase >= 1 && !res && (
            <div className="fade gpanel">
              ...
              {!ritual && <p className="gintro dim2">그래서, 요즘 뭘 망설이고 있어?</p>}
                  ...
                  {!why ? (
                    ...
                    <div className="l2 fade">
                      {detail && !detail._err
                        ...
                        : <p className="vs dim">— 이유를 불러오지 못했어 —</p>}
...
.cell.hd{padding:9px 12px;font-size:11px}
