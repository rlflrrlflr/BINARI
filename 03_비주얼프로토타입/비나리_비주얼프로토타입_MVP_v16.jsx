...
function saveMemory(m) { try { localStorage.setItem(STORE_KEY, JSON.stringify(m)); } catch (_) {} }
function clearMemory() { try { localStorage.removeItem(STORE_KEY); } catch (_) {} }
...
export default function App() {
  ...
  const [dailySeen, setDailySeen] = useState(() => { try { return localStorage.getItem(DAILY_KEY) === todayStr(); } catch (_) { return true; } }); // v16(B2)
        ...
        <section className="scene fade">
          ...
          {phase >= 1 && !res && (
            <div className="fade gpanel">
              <p className="gintro dim">{returning ? "네 조각들은 그대로 있어 — 흩어지지 않게 내가 붙들고 있었지." : "방금 맺힌 이 형상은 네 조각들로만 그려진 모습이야 — 같은 수호신은 세상에 둘 없어."}</p>
              {returning && streak && streak.count >= 2 && !res && (
                <p className="streak">수호신과 연결된 지 {streak.count}일째</p>
              )}
              {dailyData && !ritual && !res && (
                <div className="daily fade">
                  ...
                  <button className="btn ghost sm" onClick={() => { try { localStorage.setItem(DAILY_KEY, todayStr()); } catch (_) {} setDailySeen(true); }}>받았어</button>
                </div>
              )}
              {askback && !ritual && !res && (
                <div className="daily fade">
